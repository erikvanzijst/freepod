...
from app.deps import require_admin, require_self
...
admin_router = APIRouter(prefix="/usage", tags=["usage"])
...
`cost` last, with `product_id`, `product_name`, `deployment_id`,
`deployment_name`, `deployment_hostname`, `metric`, `unit` and `value` in
between for whichever dimensions are grouped. `deployment_hostname` is null
for a deployment that never had one. `value` is only present when grouping by
`metric`: quantities in different units do not add up, only their costs do.
Quantities and costs are exact, unrounded decimal strings; `currency` is the
cost's.
...
@dataclass
class UsageParams:
    """The query both usage endpoints share."""
    ...
    group_by: str | None = Query(
        None,
        description="Comma-separated dimensions to keep apart: `product`, `deployment`, "
        "`metric`. Dimensions left out are summed together; empty sums everything "
        "per bucket. Defaults to `deployment,metric` for one account and "
        "`product,metric` across accounts.",
    )
...
def _respond(session: Session, params: UsageParams, *, user_id: int | None):
    default_start, default_end = usage_report.current_month()
    group_by = params.group_by
    if group_by is None:
        group_by = "deployment,metric" if user_id is not None else "product,metric"
    report = usage_report.query_usage(
        session,
        user_id=user_id,
        start=params.start or default_start,
        end=params.end or default_end,
        bucket=params.bucket,
        group_by=_parse_csv_enum(group_by, UsageDimension, "group_by"),
        deployment_id=params.deployment_id,
        metrics={m.strip() for m in params.metric.split(",") if m.strip()}
        if params.metric
        else None,
    )
    if params.accept and "text/csv" in params.accept:
        scope = user_id if user_id is not None else "all"
        filename = f"usage-{scope}-{report.start:%Y%m%d}-{report.end:%Y%m%d}.csv"
        return Response(
...
@router.get(
    ...
    responses={**_RESPONSES, 403: {"description": "Caller may only read their own usage."}},
    description="""What an account's deployments consumed over a period, and what it costs.
...
def get_usage(
    ...
    return _respond(session, params, user_id=user_id)


@admin_router.get(
    "",
    response_model=UsageReport,
    summary="Report resource usage and its cost across every account",
    response_description="Usage per bucket and grouped dimension, as a table.",
    responses={**_RESPONSES, 403: {"description": "Administrators only."}},
    description="""What every account's deployments consumed over a period, and what it costs.

The same report as `GET /users/{user_id}/usage`, spanning all accounts, or
one when `user_id` is given. Usage no deployment is attributed to, such as
platform services, is not included.

Accounts are a filter, never a dimension, and `deployment` can only be
grouped by within one account or deployment: neither may grow with the
customer base. Group by `product` to compare across accounts.
...
"""
    + _BEHAVIOR
    + _ERRORS_400
    + """- **400 Bad Request** — grouping by `deployment` with neither `user_id`
  nor `deployment_id`.
- **403 Forbidden** — the caller is not an administrator.
""",
)
def get_all_usage(
    user_id: int | None = Query(None, description="Only this account."),
    params: UsageParams = Depends(),
