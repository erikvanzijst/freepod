...
- [x] 1.1 Add a `select_builder(source)` helper to `products/custom/builder/build.py`
...
- [x] 1.2 Log the chosen builder as the first line of the build's own output, naming the
...
- [x] 1.3 Branch `main()` between the two paths so extraction, cache refs, metadata file
...
- [x] 2.1 Add `build_and_push_dockerfile(...)` invoking `buildctl-daemonless.sh` with
...
- [x] 2.2 Pin the Dockerfile frontend: add a module constant naming the mirrored image by
...
- [x] 2.3 Mirror the frontend image into the internal registry from
...
- [x] 2.4 Confirm a project's `# syntax=` directive has no effect; verify with an
...
- [x] 2.5 Carry the unchanged import/export cache, image output, metadata-file and
...
- [x] 2.6 Confirm a failed Dockerfile build raises `BuildFailure` and publishes nothing,
...
- [x] 3.1 Add the platform port as a module constant in `build.py` whose comment names
...
- [x] 3.2 Add an image-config reader that fetches the manifest and config blob from the
...
- [x] 3.3 Emit a warning when the platform port is not among the image's declared ports
...
- [x] 3.4 Run the check on both builder paths, not only the Dockerfile one, since a
...
- [x] 4.1 Update `products/custom/builder/README.md` with the precedence rule, the absence
      ...
      `build.py` does.
- [x] 4.2 Update `cli/src/freepod/assets/SKILL.md` so a root Dockerfile is described as
...
- [x] 4.3 Bump `cli/src/freepod/__init__.py` `__version__`; verify `cd cli && uv run
      pytest` passes, and note in the change that shipping it needs a `freepod-v*` tag.

## 5. Release

- [x] 5.1 Bump `products/custom/builder/VERSION` and publish with
...
- [x] 5.2 Repoint `builder_image` in `tf/app/variables.tf` at the new version and apply;
      verify `terraform validate` passes and the build Job's `image` names the new version
      once applied.
- [x] 5.3 Deploy one project with a root Dockerfile and one without, end to end; verify the
