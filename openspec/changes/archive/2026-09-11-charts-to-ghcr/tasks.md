...
- [x] 1.4 Add publish steps for the placeholder and the charts to the `publish-images` job in `.github/workflows/ci.yml`, both with `--skip-if-published` — verify a merge to `master` with no version change leaves both steps reporting nothing to publish
...
- [x] 3.3 Roll out so catalog reconciliation runs, and verify each of the four products gained exactly one new template version whose `chart_ref` names ghcr.io, with the previous version still present
  - dev (2026-09-11): rolled out as `api:charts-to-ghcr-dev`; catalog inserted exactly one row each — custom 93→100, immich 99→101, nextcloud 98→102, vaultwarden 96→103 — all on ghcr.io, previous rows present
  - prod (2026-09-11): rolled out as `api:master` (`12d2be4`); catalog inserted exactly one row each — custom 115→124, immich 123→125, nextcloud 121→126, vaultwarden 118→127 — previous rows present

## 4. Database-authored products

- [x] 4.1 List the live templates for `helloworld`, `lemmy`, `matrix`, `mattermost` and `naas`, and record for each the `chart_ref` and `chart_version` in use — verify the recorded set accounts for every non-curated product that has at least one deployment
  - dev (2026-09-11): Hello World tpl 97 `helloworld` 0.2.1 (2 deployments); Matrix tpl 38 `matrix` 0.1.6 (0); Mattermost tpl 82 `mattermost` 1.0.15 (0); Lemmy tpl 81 `lemmy` 0.4.3 (0); NaaS has no product row. Mattermost and Lemmy trail the repo (1.1.1, 0.5.1), so on dev they move to the repo versions rather than to an address-only copy
  - prod (2026-09-11): Matrix tpl 107 `matrix` 0.1.6 (0 deployments); Hello World tpl 122 `helloworld` 0.2.1 (1); Mattermost tpl 108 `mattermost` 1.0.14 (0); Lemmy tpl 119 `lemmy` 0.5.1 (0); NaaS ("No") tpl 72 `naas` 0.1.5 (1). Mattermost and NaaS trail the repo and move to its versions — NaaS 0.1.7 renders identically and its schema only loosens
- [x] 4.2 Create a new template version per product through `caelus create-template`, identical to the one in use except for the ghcr.io `chart_ref` — verify `caelus list-templates <product_id>` shows the new version and `helm show chart` resolves the reference it names
  - dev (2026-09-11): created and made current with `update-product --template-id` (not in the task text, but required for new deployments to land on it) — Hello World 97→104, Matrix 38→105, Mattermost 82→106 (1.0.15→1.1.1), Lemmy 81→107 (0.4.3→0.5.1); system values and schema identical to the source rows; all refs resolve from the dev worker pod without credentials
  - prod (2026-09-11): Matrix 107→128, Hello World 122→129, Mattermost 108→130 (1.0.14→1.1.1), Lemmy 119→131, NaaS 72→132 (0.1.5→0.1.7), each made current; values and schema identical to the source rows, except that Hello World's JSON `null` system values became `{}`, which the platform treats identically; all refs resolve from the prod worker with TLS verification on

## 5. Deployment migration

- [x] 5.1 Enumerate live deployments grouped by product and template version, and record which template version each must move to — verify the enumeration covers every deployment that is not deleted
  - dev (2026-09-11): 8 live, all `ready`, desired = applied — Custom user app 4 on tpl 93, Hello World 2 on tpl 97, Nextcloud 1 on tpl 98, Vaultwarden 1 on tpl 96
  - prod (2026-09-11): 21 live, all `ready` — Custom user app 13 on tpl 115 (1 more already on 124), Immich 2 on 116, Nextcloud 2 on 121, Vaultwarden 1 on 118, Hello World 1 on 122, NaaS 1 on 72; no tenant workload carries a field manager other than helm, k3s or kube-controller-manager
- [x] 5.2 Move each deployment to its product's new template version and verify it reaches `ready` and serves traffic, one product at a time, starting with a single deployment per product before the rest
  ...
  - prod (2026-09-11): all 21 on ghcr.io templates — Vaultwarden →127, Nextcloud ×2 →126, Immich ×2 →125, Hello World →129, NaaS →132, Custom ×13 →124 (one more was already on 124) — each `ready` and serving its pre-move HTTP status; no tenant workload had a foreign field manager
- [x] 5.3 Verify no live deployment's rendered pod spec references the internal registry for its chart-default image, and that `custom` deployments with no build serve the ghcr.io placeholder
  - dev (2026-09-11): no pod of the 8 runs a chart-default image from the internal registry (3 `registry.home/<uid>@sha256` images are tenant build output, out of scope); `b370d4d2`, which has no build, runs `ghcr.io/…/custom-placeholder:0.1.0` with no image pull secret
  - prod (2026-09-11): no pod of the 21 runs a chart-default image from the internal registry (14 `registry.home/<uid>@sha256` images are tenant build output, out of scope); every prod `custom` deployment has a build, so none serves the placeholder

## 6. Retirement

- [x] 6.1 Audit that no product's current template and no template a live deployment desires or runs — curated or database-authored, including any backing `hello-static`, `nextcloud-wrapper` or `vaultwarden-wrapper` — and no live deployment still names the internal registry for a chart or a chart-default image (historical rows that are none of these are exempt); migrate or deliberately retire anything the audit surfaces, and verify the audit comes back empty before continuing
  ...
  - prod (2026-09-11): clean — all 21 live deployments `ready` on ghcr.io templates; none desires or runs a template naming the internal registry, and no live product's current template does; 110 historical rows across 11 products remain (exempt); no queued, running or failed reconcile jobs. No pod anywhere in the cluster runs a `registry.home/helm/*` or `registry.home/caelus/*` image, and both environments' build workers use `ghcr.io/…/builder:0.1.6`
